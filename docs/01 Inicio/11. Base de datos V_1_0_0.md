...
## 4.2. Modelo físico detallado

El siguiente diagrama complementa el modelo conceptual y representa la estructura física propuesta para PostgreSQL/PostGIS. Incluye las tablas, atributos principales, tipos de datos, claves primarias (`PK`), claves foráneas (`FK`) y restricciones de unicidad (`UK`).

```mermaid
erDiagram
    ROLES ||--o{ USUARIOS : tiene
    USUARIOS ||--o| CONDUCTORES : representa
    USUARIOS ||--o{ EJECUCIONES_OPTIMIZACION : crea
    UBICACIONES ||--o{ PEDIDOS : recibe
    UBICACIONES ||--o{ PARADAS_RUTA : localiza
    VEHICULOS ||--o{ RUTAS : utiliza
    CONDUCTORES ||--o{ RUTAS : conduce
    EJECUCIONES_OPTIMIZACION ||--o{ RUTAS : genera
    RUTAS ||--|{ PARADAS_RUTA : contiene
    PEDIDOS ||--o| PARADAS_RUTA : atiende
    FACTORES_EMISION ||--o{ METRICAS_EMISION : sustenta
    RUTAS ||--o{ METRICAS_EMISION : mide
    RUTAS ||--o{ EVENTOS_RUTA : registra
    USUARIOS ||--o{ EVENTOS_RUTA : reporta
    ...
    VEHICULOS {
        ...
        varchar combustible
        numeric consumo_base_por_100km
        numeric factor_carga_max
        smallint anio
    ...
    PEDIDOS {
        ...
        varchar destinatario_alias
        numeric demanda_kg
        numeric demanda_m3
        smallint tiempo_servicio_min
        timestamptz ventana_inicio
        timestamptz ventana_fin
        smallint prioridad
        estado_pedido estado
        varchar observacion_operativa
        timestamptz creado_en
        timestamptz actualizado_en
    }
    ...
    RUTAS {
        ...
        integer version
        estado_ruta estado
        timestamptz salida_plan
        timestamptz llegada_plan
        timestamptz salida_real
        timestamptz llegada_real
        numeric distancia_plan_km
        numeric distancia_real_km
        integer duracion_plan_min
        numeric costo_plan_pen
    ...
    PARADAS_RUTA {
        ...
        timestamptz llegada_real
        timestamptz salida_real
        numeric distancia_desde_anterior_km
        estado_parada estado
        varchar motivo_excepcion
    }

    FACTORES_EMISION {
        uuid id PK
        varchar combustible
        numeric valor_kgco2e_unidad
        varchar unidad_actividad
        varchar fuente
        varchar version_metodo
        date vigente_desde
        date vigente_hasta
        timestamptz creado_en
    }
    ...
    METRICAS_EMISION {
        uuid id PK
        uuid ruta_id FK
        uuid factor_emision_id FK
        escenario_emision escenario
        numeric actividad
        numeric consumo_equivalente
        numeric co2e_kg
        numeric incertidumbre_pct
        varchar metodologia
...
### Convenciones del modelo físico

- Las entidades usan `uuid` como clave primaria y `gen_random_uuid()` como valor por defecto.
- Las fechas y horas de eventos utilizan `timestamptz`; las coordenadas usan PostGIS con SRID 4326.
- `PK` identifica la clave primaria, `FK` una clave foránea y `UK` una restricción de unicidad.
- Los dominios `estado_usuario`, `estado_recurso`, `estado_pedido`, `estado_ejecucion`, `estado_ruta`, `tipo_parada`, `estado_parada` y `escenario_emision` se definen en el DDL de la sección 7.
- El diagrama muestra las columnas de negocio y auditoría; las restricciones `CHECK`, índices y reglas transaccionales se detallan en las secciones 6, 7, 8 y 9.

### Correspondencia entre diagramas

El modelo conceptual explica las entidades y relaciones del dominio. El modelo físico anterior muestra cómo esas entidades se materializan en PostgreSQL/PostGIS. Ambos forman parte del diseño de la base de datos y deben mantenerse sincronizados con el diccionario de datos y el DDL.
